  ...
  for (let pass = 0; pass < 3; pass += 1) {
    ...
    for (let i = 0; i < players.length; i += 1) {
      for (let j = i + 1; j < players.length; j += 1) {
        ...
        const fixY = Math.max(0.01, overlapY - slop);

        const aAboveB = a.y + PLAYER_H / 2 <= b.y + PLAYER_H / 2;
        const top = aAboveB ? a : b;
        const bottom = aAboveB ? b : a;

        // Stable stomp: only move the top player, never push the bottom player down.
        if (fixY <= 8 && top.vy >= 0) {
          top.y -= fixY + 0.01;
          top.vy = 0;
          top.onGround = true;
        ...
        // Head bump from below: stop upward motion instead of vertical shoving both players.
        if (bottom.vy < 0 && bottom.y < top.y + PLAYER_H) {
...
function stabilizeGuestPushVisuals(visuals: VisualPlayer[], targets: PlayerState[], myPlayerId: number): VisualPlayer[] {
  const myVisual = visuals.find((p) => p.id === myPlayerId);
  const myServer = targets.find((p) => p.id === myPlayerId);
  if (!myVisual || !myServer) return visuals;

  const visualById = new Map(visuals.map((p) => [p.id, p]));
  const serverById = new Map(targets.map((p) => [p.id, p]));

  for (const other of targets) {
    if (other.id === myPlayerId) continue;
    const otherVisual = visualById.get(other.id);
    const otherServer = serverById.get(other.id);
    if (!otherVisual || !otherServer) continue;

    const verticalOverlap =
      Math.min(myVisual.y + PLAYER_H, otherVisual.y + PLAYER_H) - Math.max(myVisual.y, otherVisual.y);
    if (verticalOverlap < 10) continue;

    const serverDelta = myServer.x + PLAYER_W / 2 - (otherServer.x + PLAYER_W / 2);
    if (Math.abs(serverDelta) < 0.01) continue;

    // Keep visual left/right ordering aligned with authoritative state during close body pushes.
    if (serverDelta < 0) {
      const maxMyX = otherVisual.x - PLAYER_W + 0.02;
      if (myVisual.x > maxMyX) myVisual.x = maxMyX;
    } else {
      const minMyX = otherVisual.x + PLAYER_W - 0.02;
      if (myVisual.x < minMyX) myVisual.x = minMyX;
    }
  }
...
export default function App() {
  ...
  }, [game.players, myPlayerId, role, game.mapId, cameraWidth]);
    ...
    const tick = () => {
      ...
      setRenderFrame((prev) => {
        ...
        const nextVisual = targets.map((target) => {
          ...
          const leadX = role === "guest" && isLocal ? 0.015 : 0;
          const tx = clampPlayerXToMap(target.x + target.vx * leadX, map);
          // Do not apply vertical prediction on local client: it causes land-through-then-pop visual artifacts.
          const ty = target.y;
          if (!cur) {
          ...
          const closeToLocal =
            ...
            !isLocal &&
            !!meServer &&
            Math.abs(target.x - meServer.x) < PLAYER_W * 2.2 &&
            Math.abs(target.y - meServer.y) < PLAYER_H;
          const lerpX = 1 - Math.exp(-(isLocal ? 22 : closeToLocal ? 20 : 13) * dt);
          const lerpY = 1 - Math.exp(-(isLocal ? (target.onGround ? 30 : 14) : 12) * dt);
          ...
          if (isLocal && target.onGround && finalY > ty) {
            finalY = ty;
          }
        ...
        const stabilizedVisual = role === "guest" ? stabilizeGuestPushVisuals(nextVisual, targets, myPlayerId) : nextVisual;
  ...
  }, [role, myPlayerId, game.mapId, cameraWidth]);
  ...
  const currentMap = useMemo(() => getMapById(activeGame.mapId), [activeGame.mapId]);
